...
const MARKETING_ENDPOINTS = {
  ...
  TOP_CREATING_DAYS: `${MARKETING_API_BASE}/top-creating-days`,
} as const
...
async function fetchTopCreatingDays(limit: number = 10): Promise<TopCreatingDay[]> {
  const response = await fetch(`${MARKETING_ENDPOINTS.TOP_CREATING_DAYS}?limit=${limit}`)
  const result: ApiResponse<TopCreatingDay[]> = await response.json()
  
  if (!response.ok || !result.success) {
    throw new Error(result.message || 'Failed to fetch top creating days')
  }
...
function transformTopDaysData(data: TopCreatingDay[]) {
  return data.map(item => ({
    date: item.date,
    hours: Math.round(item.total_hours),
...
// Countries data (this remains static for now)
const mockCountries = [
...
function getCountryFlagForDisplay(countryName: string): string {
  ...
  const commonFlags: Record<string, string> = {
    ...
    'Peru': '🇵🇪', 'Venezuela': '🇻🇪', 'Madagascar': '🇲🇬', 'Botswana': '🇧🇼',
...
export default function CommunityPage() {
  const downloadUrl = "https://github.com/CodeClimbersIO/ebb-app/releases/latest"
  ...
  const [topDaysLoading, setTopDaysLoading] = useState(true)
  ...
  const [topDaysError, setTopDaysError] = useState<string | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch top days
        setTopDaysLoading(true)
        const topDaysData = await fetchTopCreatingDays(10)
        setTopDays(transformTopDaysData(topDaysData))
        setTopDaysError(null)
      } catch (error) {
        setTopDaysError(error instanceof Error ? error.message : 'Unknown error')
      } finally {
        setTopDaysLoading(false)
      }
    }
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-background">
      <div className="container mx-auto max-w-7xl px-4 pt-32 pb-12 space-y-8">
        <div className="text-center space-y-4">
          <div className="inline-block relative">
            <div className="absolute -left-8">
              <div className="text-sm text-yellow-400 font-semibold -rotate-[35deg]">ebb.cool</div>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold bg-gradient-to-r from-violet-400 to-violet-600 bg-clip-text text-transparent">
              Community
            </h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {/* Weekly Creating Time Line Chart */}
        ...
        {/* Call to Action */}
        <Card className="bg-gradient-to-r from-violet-900/20 to-blue-900/20 border-violet-500/20">
          <CardContent className="pt-6">
            <div className="text-center space-y-4">
              <h2 className="text-2xl font-bold">Join the Creating Community</h2>
              <p className="text-muted-foreground max-w-2xl mx-auto">
                Ready to boost your focus and join thousands of creators? Download Ebb and start tracking your creating time today.
              </p>
